...
Tinky にはローカル計測用スクリプトと CI 相当の閾値チェックが用意されていま
す。差分ロジックを変更したときは両方を実行してください。

1. `bun run perf:render` を実行して計測サンプルを確認します。
2. `bun run perf:gate` を実行して CI と同じゲートを確認します。
3. line-diff と run-diff の中央値を比較します。
